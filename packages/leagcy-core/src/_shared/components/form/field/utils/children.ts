...
import { flattenChildren, logger } from '../../../../utils'
...
function getCollectFunction(
  ...
  const { rule, trigger, validateTrigger, formatter, getValueProps, getValueFromEvent } = props

  return (childProps: AnyObj = {}) => {
    const value = instance.getFieldValue(control._name)
...
export function normalizeChildren(
  ...
  const collected = handleCollect((children as any).props)
