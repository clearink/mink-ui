...
export class FormDispatchControl<State = any> {
  ...
  validateFields = async (nameList?: ExternalNamePath[]) => {
    const { $state, $controls } = this

    // 所有的校验字段
    const controls = $controls.getValidateFieldsControls(nameList)

    const promiseList = controls.map(async ctrl => ctrl.validate($state.getFieldValue(ctrl._name)))

    const validatePromise = Promise.resolve().then(async () => {
    ...
    const returnPromise = validatePromise.then<State>(async (errors) => {
      this.triggerOnFieldsChange(controls)
