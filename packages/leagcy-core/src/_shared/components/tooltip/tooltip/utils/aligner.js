...
/* ****************************** screen coords ****************************** */
function getTopOrBottomScreenCoords(main, cross) {
...
function getLeftOrRightScreenCoords(main, cross) {
    return (props, popup, trigger) => {
...
/* ****************************** popup offset ****************************** */
function offsetPopupCoords({ offset }, popup) {
...
/* ****************************** shift coords ****************************** */
// 调整上，下的水平方向
function shiftTopOrBottomPopupCoords() {
    const getLeftCoords = (popup, trigger) => {
        const min = (_px + _size) * 2 + popup._delta;
        const max = popup._mx - popup._width;
        if (popup.left <= 0)
            return Math.min(trigger.right - min, 0);
        if (popup.left >= max)
            return Math.max(trigger.left - popup._width + min, max);
    };
    return (props, popup, trigger) => {
        const { shift } = props;
        if (!shift || (isObject(shift) && shift.horizontal === false))
            return popup;
...
// 调整左，右的竖直方向
function shiftLeftOrRightPopupCoords() {
    const getTopCoords = (popup, trigger) => {
        const min = (_py + _size) * 2 + popup._delta;
        const max = popup._my - popup._height;
        if (popup.top <= 0)
            return Math.min(trigger.bottom - min, 0);
        if (popup.top >= max)
            return Math.max(trigger.top - popup._height + min, max);
    };
    return (props, popup, trigger) => {
        const { shift } = props;
        if (!shift || (isObject(shift) && shift.vertical === false))
            return popup;
...
/* ****************************** flip coords ****************************** */
// 翻转上，下主轴
function flipTopOrBottomPopupCoords() {
    const getTopCoords = (popup, trigger) => {
        if (popup.main === 'top' && popup.top > 0)
            return;
        const bottom = popup.top + popup._height;
        if (popup.main === 'bottom' && bottom < popup._my)
            return;
        // 保证有足够的空间 ？
    ...
    return (props, popup, trigger) => {
        const { flip } = props;
        if (!flip || (isObject(flip) && !flip.vertical))
            return popup;
        const top = getTopCoords(popup, trigger);
...
// 翻转左，右主轴
function flipLeftOrRightPopupCoords() {
    const getLeftCoords = (popup, trigger) => {
        if (popup.main === 'left' && popup.left > 0)
            return;
        const right = popup.left + popup._width;
        if (popup.main === 'right' && right < popup._mx)
            return;
        return trigger.right + trigger.left - right;
    };
    return (props, popup, trigger) => {
        const { flip } = props;
        if (!flip || (isObject(flip) && !flip.horizontal))
            return popup;
        const left = getLeftCoords(popup, trigger);
...
/* ****************************** arrow coords ****************************** */
function getTopOrBottomArrowCoords() {
    return (popup, trigger) => {
        ...
        let left = -popup.left;
        if (popup.cross === 'left')
            left += min + _px + popup._delta;
        else if (popup.cross === 'right')
            left += max - dx - popup._delta;
        else
            left += (max + min) / 2 - _size;
        return { left, top, transform: `rotate(${rotate}deg)` };
    };
}
function getLeftOrRightArrowCoords() {
    return (popup, trigger) => {
        ...
        let top = -popup.top;
        if (popup.cross === 'top')
            top += min + _py + popup._delta;
        else if (popup.cross === 'bottom')
            top += max - dy - popup._delta;
        else
            top += (max + min) / 2 - _size;
        return { left, top, transform: `rotate(${rotate}deg)` };
    };
...
/* ****************************** coords getter ****************************** */
function makeArrowCoordsGetter(curr) {
    return (prev) => (isEqual(prev, curr) ? prev : curr);
}
function makePopupCoordsGetter(curr) {
    return (prev) => (isEqual(prev, curr) ? prev : curr);
}
/* ****************************** aligner ****************************** */
function aligner(main, cross) {
    const isTopOrBottomAlign = main === 'top' || main === 'bottom';
    const getScreenCoords = isTopOrBottomAlign
        ? getTopOrBottomScreenCoords(main, cross)
        : getLeftOrRightScreenCoords(main, cross);
