import { useConstant, useExactState, useForceUpdate } from '../../../_shared/hooks';
...
export default function useNotificationList(props, stackEnable, stackConfig) {
    ...
    const runHoverCleanup = () => {
        ...
        refs.hovers.forEach((key) => { if (!set.has(key))
            refs.hovers.delete(key); });
        if (refs.hovers.size !== oldCount)
            forceUpdate();
    };
